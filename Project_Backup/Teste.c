...
#fuses xt,nowdt,noprotect,put,nobrownout,nolvp // configuração dos fusíveis 
...
#bit    LENA = porte.1
#bit    LDAT = porte.0
#bit    S1 = portb.0// definição das entradas
#bit    S2 = portb.1// definição das entradas
#bit    S3 = portb.2// definição das entradas
#byte   LPORT = portd
/************************************************************************************/
/*                        Definições                                 */
/************************************************************************************/
...
#define L_CFG   0x38
/************************************************************************************/
unsigned char TimerEepromHandler;//Variável de controle da frequência de leitura
...
void delay(void);
char Int_Struct(char d); // função implementada
...
/*******************************************************************/
/* Nome da Função:  Int_Struct                                   */
/* Proposito: Inicializa/configura estruturas e atualiza mensagem inicial*/
/* Entradas:  char d - valor do case desejado                          */
/* Saídas:  char d - variável de controle da atualização          */
/*******************************************************************/
char Int_Struct(char d)
{        
   switch(d)
   {
      case 0: //Inicializa estrutura que receberá os valores lidos na eeprom
  ...
  while(true)
   {
      if(!TimerEepromHandler)
         { 
         d=Int_Struct(d); // função que atualiza LCD
         if(!S1)
               { 
                for(m=0;m<2;m++)
                    {                      
